...
public class OceanPanel extends JPanel {
    ...
    /**
    * Constructor, set up the size, the bounds
    * setup timer, setup background, and add mouseListener
    *
    ...
    */
    ...
    public void checkDead() {
        // remove the ones that are dead (their health is
        // equal or less than zero)
        for (int i = 0; i < fishies.size(); i++) {
            Fish fish = fishies.get(i);
            if (fish.getHP() <= 0) {
                 fishies.remove(fish);
            }
        }
    ...
    public void euthanize() {
        ...
        //each call will cause all fishes get older
        for (Fish fish: fishies) {
            fish.getOlder();
        }
        //kill the fish whose age is large than 120
        for (Fish fish: fishies) {
            if (fish.getAge() > 120) {
                fish.setHPToZero();
            }
    ...
    /**
     * move all fishes, checkReproduce,
     * checkFoodChain and checkDead
